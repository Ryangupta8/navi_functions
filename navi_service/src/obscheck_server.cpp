...
#include <navi_service/ObsCheckerAction.h> 
...
class ObsCheck_Server
{
public:
    
  ObsCheck_Server(std::string name): 
  as_(nh_, name, boost::bind(&ObsCheck_Server::executeCB, this,_1), false),
  action_name_(name),
  IsGoal(false),
  IsRotated(false),
  IsActive(false),
  ...
  srv_time(0.0)
  {
      ...
      dynamicmap_sub =nh_.subscribe<nav_msgs::OccupancyGrid>("/dynamic_obstacle_map_ref", 30, &ObsCheck_Server::dynamicmapCallback,this); 
      globalpose_sub=nh_.subscribe<geometry_msgs::PoseStamped>("/global_pose",10,&ObsCheck_Server::global_pose_callback,this);
    ...
    //as_.registerPreemptCallback(boost::bind(&ObsCheck_Server::preemptCB, this));
    //subscribe to the data topic of interest
    as_.start();
    
  }

  ~ObsCheck_Server(void)
  {
  }
  ...
  void executeCB(const navi_service::ObsCheckerGoalConstPtr &goal)
  {
     // ros::Rate r(1);
...
protected:
  ...
  actionlib::SimpleActionServer<navi_service::ObsCheckerAction> as_;
  navi_service::ObsCheckerFeedback feedback_;
  navi_service::ObsCheckerResult result_;
...
{
  ros::init(argc, argv, "obscheck_action");
  ObsCheck_Server obs_manager(ros::this_node::getName());
  double ros_rate = 1;
  ros::Rate r(ros_rate);
